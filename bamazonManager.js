	...
	inquirer.prompt([
		{
			type: 'input',
			name: 'item_id',
			message: 'Please enter the Item ID you would like to add more items to: ',
		...
		connection.query(queryStr, {item_id: item}, function(err, data) {
			...
			if (data.length === 0) {
				console.log('ERROR: Invalid Item ID. Please select a valid Item ID and try again!');
				addInventory();
				...
				connection.query(updateQueryStr, function(err, data) {
					...
					console.log('Current stock_quantity for Item ID: ' + item + ' has been updated to: ' + (productData.stock_quantity + addQuantity) + ' items!');
